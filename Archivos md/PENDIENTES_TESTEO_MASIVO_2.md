...
| G7 | **Separación notificaciones estudiante vs familia** | Las notificaciones del familiar deben separarse: en el apartado de notificaciones del **estudiante** solo van las académicas; en las **personales** del familiar van las suyas. Las que no aparezcan en las del estudiante figuran en personales (M1). | ✅ Hecho |
...
| C2 | Preceptor | ✅ | Hecho: el crash era por `toggleAlumno`, que pasaba un updater `(prev) => ...` a un `setAlumnosIds` que espera el valor directo (quedaba una función guardada en `form.alumnos_ids` y `.includes` reventaba al renderizar). Ahora calcula desde el prop `alumnos_ids`. | `Shared/TutoresEstudiantesEditor.jsx` |
...
| C7 | Admin | ✅ | Corregido (selector de materia en RITE). | — |
...
|---|---|---|---|---|
| A1 | Todos | ✅ | Funciona. | — |
| A2 | Estudiante/Familia | ✅ | Lo hizo bien. | — |
...
| A7 | Docente/Admin | ✅ | Hecho: `_notificar_carga_notas` ahora envía **una notificación por nota (por estudiante)** a directivos y preceptores, con `dedupe_key` por nota. | `views.py` (`_notificar_carga_notas`), `Profesores/PanelEstudiantes.jsx` |
...
| A12 | Admin | ✅ | Creó bien los botones de acción de estudiantes. | — |
...
| A14 | Admin | ✅ | Funciona. G5 aplicada: leyenda de emojis unificada en `AccionesLeyenda.jsx`, presente en todos los apartados con los mismos botones (Preceptores docentes/estudiantes/tutores, AdminPreceptores sin programar, administradores, preceptores). | `Shared/AccionesLeyenda.jsx` |
...
| A19 | Jefe | ✅ | Hecho: la Jefatura muestra los **tres apartados** (actas de estudiantes, de docentes y de curso); `Preceptores/actas.jsx` excluye las actas de estudiantes de la lista de actas de curso y `DataContext.jsx` filtra los links huérfanos de actas dadas de baja (eran las filas en blanco). | `JefePreceptores/JefePreceptorDashboard.jsx`, `Preceptores/actas.jsx`, `DataContext.jsx` |
...
| # | Rol | Estado | Problema / Verificación y qué falta | Dónde |
|---|---|---|---|---|
| M1 | Estudiante/Familia | ✅ | Backend verificado: `_notificar_acta_curso` genera la notif del acta de curso al estudiante como **académica** (id_alumno seteado, segmento estudiante) y a la familia como **personal** (id_alumno nulo, segmento familia). Se corrigieron 2 bugs que rompían E21 (ver Registro). **Confirmación visual del cliente**: las notificaciones llegan en el panel del estudiante y en la parte de personales de familia. | `views.py` (`_notificar_acta_curso`) |
| M2 | Estudiante/Familia | ✅ | Ok. | — |
| M3 | Estudiante/Familia | ✅ | Verificado en transacción (rollback): `_notificar_recursada` emite "Recursada cargada" al cargar y "Resultado de recursada" (APROBADA/DESAPROBADA) al cambiar de estado; llega al estudiante y a sus 5 familiares. | `views.py` (`_notificar_recursada`) |
| M4 | Estudiante/Familia | ✅ | Aclarado: se refiere al evento **E19** "bloqueo de horario por superposición" (recursada superpuesta al horario regular). Verificado en transacción (rollback): `_notificar_bloqueo_horario` crea la notificación "Bloqueo de horario por superposición" al **estudiante** (1) y a sus **5 familiares** (segmento familia). | `views.py` (`_notificar_bloqueo_horario`), `BloqueoHorarioAlumnoViewSet` |
| M5 | Estudiante | ✅ | Funciona. | — |
| M6 | Estudiante/Familia | ✅ | Ok. | — |
| M7 | Familia | ✅ | Se ven. | — |
...
| M13 | Preceptor | ✅ | Hecho (confirmado por el cliente: "arreglado tanto en preceptores como en jefe de preceptores"): `Preceptores/actas.jsx` ya no mezcla actas de estudiantes en las de curso y se filtran las filas en blanco (links a actas dadas de baja). | `Preceptores/actas.jsx`, `DataContext.jsx` |
| M14 | Jefe | ✅ | Llega bien. | — |
| M15 | Jefe | ✅ | Llega bien. | — |
...
| B4 | Jefe | ✅ | Hecho: `EstadisticasPreceptoria.jsx` rediseñado con **panel de cobertura** (cursos con preceptor %, barra de progreso), tarjetas-kpi (total preceptores, cursos con/sin preceptor, estudiantes, tutores) y chips dinámicos. Build OK. | `EstadisticasPreceptoria.jsx` |
...
| B6 | Jefe | ✅ | Hecho: era bug de backend. El usuario jefe de prueba tiene también el rol `familia` y `AlumnoViewSet.get_queryset` lo restringía a los hijos de su tutor (sin tutor → lista vacía). Ahora los roles amplios (`es_rol_amplio`: admin/director/jefe_preceptores/preceptor) se evalúan **antes** que familia/docente. Verificado vía API: el jefe ve **274 estudiantes**. | `views.py` (`AlumnoViewSet.get_queryset`), `JefePreceptores/` (estudiantes) |
| B7 | Jefe | ✅ | Ok. | — |
| B8 | Jefe | ✅ | Ok. | — |
...
| B10 | Estudiante/Jefe/Admin | ✅ | Funciona. | — |
...
| B15 | Performance | ✅ | Hecho (cubierto cruzadamente por B13/B14 = misma causa raíz): lo que hacía lenta la creación/edición de estudiantes era el **refresco global tras cada guardado**, que disparaba la bajada completa de la tabla de asistencias (la petición más pesada, unbounded). Al reemplazarla por `Promise.resolve([])` en el `DataContext`, el guardado de un estudiante ya no espera esa descarga. Pendiente: confirmación visual en el navegador. | `DataContext.jsx` (mismo cambio que B13/B14), `Preceptores/estudiantes.jsx` |
...
|---|---|---|
| `Preceptores/tutoresFamilias.jsx` | C2 | ✅ corregido (crash al seleccionar estudiante) |
...
| `PanelMateriasAdeudadasDocente.jsx` / `ActividadesView.jsx` | A4 | ✅ intensificación + formato estudiante/familia + filtro global |
| `Preceptores/actas.jsx` | A19, M13 | ✅ actas de curso sin actas de estudiantes + sin filas en blanco |
...
| `Preceptores/estudiantes.jsx` | B15 | ✅ guardado de estudiante sin descarga global de asistencias (mismo cambio B13/B14) |
...
- **Críticos C1–C3**: contador de notificaciones por hijo; crash de "Nuevo Tutor" (`TutoresEstudiantesEditor`); materias filtradas por curso + permiso `PuedeGestionarCursoMateria` para preceptor (+ validación de asignaciones).
- **A3**: los comunicados se filtran por curso+materia (alcances e `id_materia`).
- **A4**: materias adeudadas (previa **e intensificación**) visibles para docente/estudiante/familia, con diferenciación visual (rojo/naranja), detalle clickeable y filtro global (curso/hijo).
...
- **A7**: la carga de notas notifica **una por nota (por estudiante)** a directivos y preceptores.
- **A15**: reactivar cursos corregido (filtro `activo` solo en el listado).
- **A19 + M13**: actas de curso sin actas de estudiantes (jefe/preceptor) y sin filas en blanco.
...
- **Globales G1–G7**: cierre de formularios, auto-refresco tras acciones, mensajes entendibles, navegación de desplegables, leyenda de acciones unificada, agrupación de notificaciones y separación estudiante/familia.
...
- **M3**: verificado `_notificar_recursada` ("Recursada cargada" y "Resultado de recursada") hacia el estudiante y sus familias.
- **Bug E21 (2)**: `_notificar_acta_curso` fallaba con `NameError` (`SEGMENTO_ALUMNO`/`SEGMENTO_FAMILIA` sin importar) y con `ValueError` (`id_alumno` recibía un int en vez de la instancia `Alumno`). Ambos corregidos; el flujo de acta de curso generó 45 notifs en test (18 estudiantes con vínculo académico, 15 familiares como personales, + preceptores/jefes/directivos).
...
- **M4**: aclarado que "bloqueo de horarios" = evento **E19** (bloqueo por superposición de horario). Verificado `_notificar_bloqueo_horario` en transacción: notifica al estudiante (1) + 5 familiares.
