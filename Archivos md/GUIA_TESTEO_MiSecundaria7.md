...
**Estudiantes / familias:** `alumno_001_prueba`…`alumno_270_prueba` y `familia_001_prueba`…`familia_120_prueba` (`123`). Preferidos para este testeo:

| Usuario | Contraseña | Rol | Persona asociada | Datos precargados |
|---|---|---|---|---|
| `alumno_001_prueba` | `123` | Estudiante | Prueba001 Alumno001 (1°1) | 4 calificaciones, 3 asistencias |
| `alumno_002_prueba` | `123` | Estudiante | Prueba002 Alumno002 (1°1) | 2 calificaciones, 3 asistencias |
...
- **Cursos activos**: los 18 cursos de 1° a 6° división 1-3 (1°1 … 6°3), ~15 estudiantes por curso (270 estudiantes en total).
...
- **Tutores/familias/familias**: `familia_00N_prueba` suelen tener 3 hijos vinculados (uno de 1°, uno de 3°, uno de 6°) para probar el selector.
...
3. La **deduplicación anti-spam** bloquea notificaciones idénticas al mismo usuario dentro de 5 minutos. Para probar una misma acción 2 veces seguidas, cambiar el contenido (ej. distinta nota, distinto estudiante, distinto mensaje).
...
## 1. Estudiante — `alumno_001_prueba` / `123`

**Iniciar sesión → Seleccionar rol "Estudiante"** (si el sistema lo pide).
...
| 9 | **Buscar un estudiante que no existe / URL inválida** | Que no crashee, muestre mensaje amigable. |
...
- [ ] **Prenota registrada**: cuando el docente guarda una prenota (TEP/TEA/TED) del estudiante.
...
- [ ] **E13 — Promoción / no promoción** (RITE).
...
**NO debe recibir:** actas de curso (E21), cargas masivas de notas/asistencias, alta de otros usuarios (E20), ni las notificaciones de calificaciones/inasistencias/actividades de **otros** estudiantes.
...
| 11 | Cambiar de hijo en campana | Con el filtro de estudiante activo, que solo aparezcan las notificaciones del niño elegido. |
...
- [ ] **E13 — Promoción / no promoción** (RITE) de cada hijo.
...
**NO debe recibir:** actas de curso (E21), cargas masivas, alta de usuarios (E20), ni datos de estudiantes que **no** sean sus hijos.
...
| 7 | **Selector de curso/materia** | Cambiar entre Matemática 1°1, Artística 2°3 y otras asignadas → que todo (asistencia, estudiantes, libro de temas) cambie acorde. |

### 3.2. Acciones con notificación (especificar destino)

| # | Acción a realizar | Qué revisar | Notificación y destinatarios |
|---|---|---|---|
| 8 | Cargar una **calificación** a Prueba001 Alumno001 (Matemática 1°1) y guardar | Que la nota se guarde y aparezca en sus calificaciones | **→ Estudiante Prueba001** (alumno_001_prueba) **y su familia** (familia_002_prueba): "Calificación cargada". Navega a Calificaciones. Probar también **modificar una nota existente** (debe volver a notificar). |
| 9 | Cargar una **prenota (previa guardada como TED/TEP/TEA)** | Que quede registrada | **→ Estudiante y familia** del estudiante: "Prenota registrada". |
| 10 | Registrar **inasistencia "Ausente"** a un estudiante del curso | Que quede en Asistencia y aparezca en el historial del estudiante | **→ Estudiante y familia**: "Inasistencia registrada" (se acumulan en un solo aviso por día si hay varias ausencias ese día). |
| 11 | **Publicar una actividad** (tarea/trabajo) con consigna y adjunto | Que se publique y se vea en Actividades del curso | **→ Estudiantes del curso + sus familias**: "Actividad publicada". Navega a Actividades. Probar **editar la actividad** (debe volver a notificar). |
| 12 | Crear una **actividad para materia adeudada** (si existe estudiante con materia adeudada en el curso) | Que se asocie correctamente | **→ Estudiantes con MateriaAdeudada en estado ADEUDADA/RECURSANDO + sus familias**. |
...
| 15 | **Marcarse ausente (falta de docente)** en la asistencia de docentes | Que quede registrada su falta | **→ Estudiantes de sus cursos + familias + Jefe de Preceptores**: "Falta de docente". |
...
| 17 | **Cambiar/crear un horario** de su materia (si tiene permiso) | Que el horario se actualice | **→ Estudiantes del curso + familias + docente de la materia + preceptores + directivos**: "Horario modificado". |

### 3.3. Notas específicas

- La acción de **"Suplencia asignada"**: si el admin asigna a Ana como docente suplente → **Ana (docente_02_prueba) recibe** "Suplencia asignada" que navega a su **perfil docente** (donde se listan sus suplencias); y los **estudiantes del curso + familias** también reciben aviso.
- **Adelanto de horas aprobado** vinculado a sus materias → le llega a él como docente titular, a preceptores, directivos, jefe y a los **estudiantes del curso + familias**.
...
- [ ] **Falta de docente**: **NO** te llega a vos como docente, sino a los estudiantes del curso + familias + Jefe de Preceptores (verificarlo con `docente_05_prueba` → `preceptor_1_prueba` NO recibe; los estudiantes 1°1 SÍ).
...
**NO debe recibir:** actas de conducta (E4, van a estudiante+familia+preceptores+jefe), actas de curso (E21), cargas masivas de notas/asistencias (van a directivos+preceptores), alta de usuarios (E20), ni las calificaciones de estudiantes.
...
| 2 | **Estudiantes** | Ver listado de estudiantes; buscar por curso, nombre, DNI. Probar **crear** y **modificar** un estudiante. | Si el alta crea un usuario nuevo → **Preceptores + Jefe + Directivos** (E20, sin el actor). |
| 3 | **Tutores/familias/familias** | Ver tutores; agregar/editar un tutor y vincularlo a un estudiante (estudiantes sin tutor). |
| 4 | **Docentes** | Ver listado de docentes del establecimiento. |
| 5 | **Panel Diario** | Revisar el panel del día: clases, módulos, ausencias de docentes, estudiantes. |
| 6 | **Asistencias** | Registrar asistencia de un curso: marcar **Ausente** a un estudiante y guardar. | **→ Estudiante y familia**: "Inasistencia registrada". Registrar varias ausencias el mismo día → debe ser **un solo** aviso acumulado. |
...
| 10 | **Acta de conducta/apercibimiento** a un estudiante (si puede crearla) | | **→ Estudiante + familia + preceptores del curso + Jefe de Preceptores**: "Acta de conducta". |
| 11 | **Horarios** | Ver horarios por curso. |
| 12 | **Adelantos de Horas** | Crear un adelanto de horas para una materia de un curso. | **→ Docente titular + preceptores + directivos + Jefe + estudiantes del curso + familias**: "Adelanto de horas". |
| 13 | **Asistencias — marcar a un docente como Ausente** (falta de docente) | En Asistencias, registrar al docente de la materia como **Ausente** para el día. | **→ Estudiantes del curso + familias + Jefe de Preceptores**: "Falta de docente". (El preceptor NO gestiona suplencias: eso es del Admin.) |
...
- [ ] **Actas de conducta** (E4) de estudiantes de tus cursos → también llegan a estudiante+familia+jefe.
- [ ] **E20 — Alta de usuario nuevo** (estudiante/docente/preceptor creado por otro usuario): llega a preceptores de cursos + jefe + directivos (**NO** a vos si fuiste el actor que lo creó).
- [ ] **E15 — Adelanto de horas** aprobado en cualquiera de tus cursos → "Adelanto de horas" (llega a docente titular + preceptores + directivos + jefe + estudiantes + familias).
- [ ] **E17 — Suplencia asignada** en un curso que está a tu cargo → preceptores del curso + docente suplente + directivos + estudiantes + familias. (El preceptor gestiona la parte de sus cursos, no la suplencia.)
...
- [ ] **Cambio de horario** de tus cursos → "Horario modificado" (estudiantes + familias + docente + preceptores + directivos).
- [ ] **Comunicado alcance General**.
- [ ] **Comunicado alcance curso** de 1°1, 1°2 o 1°3.
- [ ] **Comunicado alcance materia** de una materia de tus cursos.
- [ ] **E16 — Evento institucional**.
- [ ] **E14 — Cuenta habilitada/deshabilitada**: solo si se deshabilita/habilita tu propia cuenta.
- [ ] **Falta de docente** en tus cursos: **NO** te llega a vos como preceptor; llega a estudiantes del curso + familias + **Jefe de Preceptores** (verificar que no aparece aunque marques Ausente a un docente).

**NO debe recibir:** calificaciones/pre-notas de estudiantes (van a estudiante+familia), inasistencias de estudiantes (E3, son del estudiante+familia), suplencias de cursos que NO son tuyos, ni las notificaciones del día de otros preceptores.
...
| 6 | **Estudiantes** | Ver listado de estudiantes. |
| 7 | **Asistencias** | Ver asistencias por curso. |
| 8 | **RITE / Notas** | Ver notas/RITE por curso. |
| 9 | **Actas** | Ver actas. |
| 10 | **Adelantos de Horas** | Ver/ingresar adelantos. |
| 11 | **Docentes / Tutores/familias/familias** | Ver listados de docentes y tutores. |
...
- **Acta de conducta** → jefe (además de estudiante/familia/preceptor).
- **Falta de docente** → jefe (además de estudiantes/familias).
...
- [ ] **E4 — Acta de conducta / apercibimiento** de cualquier estudiante → te llega (además de estudiante, familia y preceptor del curso).
- [ ] **Falta de docente** (cuando un docente se marca Ausente en el día) → "Falta de docente" (estudiantes del curso + familias + jefe).
- [ ] **E15 — Adelanto de horas** aprobado en cualquier curso → "Adelanto de horas".
- [ ] **E20 — Alta de usuario nuevo** (estudiante/docente/preceptor creado por otro) → preceptores + jefe + directivos (NO si sos el actor).
- [ ] **Comunicado alcance General**.
- [ ] **Comunicado alcance curso** (de cualquier curso).
- [ ] **Comunicado alcance materia**.
- [ ] **E16 — Evento institucional**.
- [ ] **E14 — Cuenta habilitada/deshabilitada**: solo si se deshabilita/habilita tu propia cuenta.
- [ ] **E3 / E1 — Inasistencias y calificaciones de estudiantes**: **NO** te llegan (van a estudiante + familia). Verificar que no aparezcan.
...
|---|---|---|---|
| 1 | **Perfil** | Datos propios. |
| 2 | **Estudiantes** | Crear, editar, buscar, eliminar/desactivar un estudiante. Probar adjuntarle tutor y usuario. | **Crear usuario estudiante** → preceptores + jefe + directivos (E20), excluyendo al actor. |
...
| 8 | **Suplencias Docentes** | Crear una suplencia para un docente. | **→ Docente suplente + preceptores del curso + directivos + estudiantes del curso + familias**: "Suplencia asignada". |
...
| # | Acción a realizar | Qué revisar | Notificación |
|---|---|---|---|
| 10 | **Horarios** | Crear/editar/eliminar un horario para un curso. | **→ Estudiantes del curso + familias + docente de la materia + preceptores + directivos**: "Horario modificado". |
| 11 | **Adelantos de Horas** | Crear un adelanto. | **→ Docente titular + preceptores + directivos + jefe + estudiantes del curso + familias**: "Adelanto de horas". |
| 12 | **Asistencias** | Cargar asistencia en bloque / por curso; marcar ausencias. | **Inasistencias** → estudiante y familia. **Carga masiva** → directivos + preceptores del curso. |
| 13 | **Notas (RITE)** | Cargar/editar notas. Revisar que el RITE se calcule y actualice. | **Calificación** → estudiante + familia. **RITE consolidado/promoción** → estudiante + familia (E13). |
| 14 | **Actas** | Crear actas de curso/estudiante. | **Acta de curso** → preceptores + jefe. |
...
| # | Acción a realizar | Qué revisar | Notificación |
|---|---|---|---|
| 16 | **Publicar Comunicado — alcance General** | Que todos los roles puedan verlo. | **→ Todos**: estudiantes + familias + docentes + preceptores + jefe + directivos: "Nuevo comunicado". |
| 17 | **Publicar Comunicado — alcance curso (ej. 1°1)** | Que solo lo vea 1°1. | **→ Estudiantes de 1°1 + sus familias + docentes con materias en 1°1 + preceptor de 1°1 + jefe + directivos**. NO debe llegar a 1°2 ni 2°1. |
| 18 | **Publicar Comunicado — alcance materia (ej. Matemática)** | Que solo lo vean los inscriptos en Matemática. | **→ Estudiantes con Matemática en ese curso + familias + docente de Matemática + preceptores + jefe + directivos**. |
...
| 23 | Reprocesar / verificar RITE (comandos o botones disponibles) | Que no haya errores. |
...
- [ ] **Falta de docente**: **NO** te llega como directivo (va a estudiantes + familias + jefe). Verificar que no aparezca.
- [ ] **E15 — Adelanto de horas** aprobado → directivos.
- [ ] **E17 — Suplencia asignada** → "Suplencia asignada" (directivos + docente suplente + preceptores del curso + estudiantes + familias).
...
- [ ] **E4 — Acta de conducta**: **NO** debería llegarte (va solo a estudiante + familia + preceptores del curso + jefe).

**NO debe recibir (cont.):** calificaciones/pre-notas/inasistencias de estudiantes (E1/E6/E3 van a estudiante+familia), faltas de docente (estudiantes+familias+jefe), ni notificaciones de los cursos ajenos a tu gestión.
...
**NO debe recibir (cont.):** falta de docente (estudiantes+familias+jefe), calificación/inasistencia de estudiantes (E1/E3 → estudiante+familia).
...
|---|---|---|
| Docente/Admin carga calificación | E1/E6 | Estudiante + familia del estudiante |
| Prenota guardada | — | Estudiante + familia |
| Inasistencia "Ausente" | E3 (DAILY) | Estudiante + familia (un aviso/día, acumulado) |
| Acta de conducta/apercibimiento | E4 | Estudiante + familia + preceptores del curso + jefe |
| Comunicado publicado | E7 | Según alcance (General → todos; curso → estudiantes+familias+docentes+preceptores del curso + jefe + directivos; materia → inscriptos) |
| Planificación a revisión | E8 | Admin + director |
| DDJJ presentada | E9 | Admin + director |
| Materia pasa a previa | E10 | Estudiante + familia |
| Rendición registrada | E11 | Estudiante + familia |
| Intensificación | E12 | Estudiante + familia |
| Promoción/no promoción (RITE) | E13 | Estudiante + familia |
| Usuario habilitado/deshabilitado | E14 | Usuario afectado |
| Adelanto de horas aprobado | E15 | Docente titular + preceptores del curso + directivos + jefe + estudiantes del curso + familias |
| Evento institucional | E16 | TODOS (estudiantes, familias, docentes, preceptores, jefe, directivos) |
| Suplencia asignada | E17 | Docente suplente + preceptores del curso + directivos + estudiantes del curso + familias |
| Recursada | E18 | Estudiante + familia |
| Bloqueo horario (crear/levantar) | E19 | Estudiante + familia |
...
| Actividad publicada/actualizada | — | Estudiantes del curso + familias |
| Actividad de materia adeudada | — | Estudiantes con MateriaAdeudada + familias |
| Horario creado/modificado/eliminado | — | Estudiantes del curso + familias + docente de la materia + preceptores + directivos |
| Falta de docente (Ausente) | — | Estudiantes del curso + familias + jefe |
...
5. **Filtros y búsquedas**: curso, materia, período, estudiante — en cada pantalla que los tenga.
...
- [ ] Docente: cargó calificación y **estudiante + familia** recibieron la notificación.
- [ ] Docente: publicó actividad y **estudiantes del curso + familias** recibieron la notificación.
- [ ] Docente: presentó planificación/DDJJ y **admin + director** la recibieron.
- [ ] Preceptor: registró ausencia y **estudiante + familia** la recibieron (acumulada por día).
- [ ] Preceptor: creó acta de conducta → **estudiante + familia + preceptores + jefe**.
...
- [ ] Estudiante y Familia: recibieron y pudieron navegar desde cada notificación al módulo correcto.
...
| 4 | Botones "Nuevo Tutor" / "Nuevo Docente" cuelgan la página | ✔ Corregido | `renderFormTutor()` se invocaba sin estar definido → ReferenceError durante el render. Ídem `<AsignacionesEditor />` en docentes. Se crearon `renderFormTutor`, `TutoresEstudiantesEditor.jsx` y `AsignacionesEditor.jsx`. `npm run build` OK. |
