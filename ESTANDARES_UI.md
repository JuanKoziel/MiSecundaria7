  # ESTÁNDARES UI — Guía oficial de diseño y desarrollo visual
  ...
  ### Variables CSS definidas en `:root` (líneas 4–25 de `index.css`)

  | Variable | Valor | Uso |
  |----------|-------|-----|
  | `--primary-color` | `#fd7e14` | Botones, acentos, focus rings |
  | `--primary-dark` | `#e06d0f` | Estados hover de botones |
  | `--bg-gradient` | `linear-gradient(...)` | Fondo del body |
  | `--sidebar-color` | `#0d233a` | Fondo del sidebar |
  | `--sidebar-hover` | `#163452` | Hover del sidebar, contenedores de formularios oscuros |
  | `--text-dark` | `#333333` | Texto principal |
  | `--text-light` | `#666666` | Texto secundario, labels |
  | `--white` | `#ffffff` | Fondos de cards |
  | `--shadow` | `0 4px 15px rgba(0,0,0,0.05)` | Sombras de cards y headers |
  | `--radius` | `12px` | Border radius general |
  | `--transition` | `all 0.3s ease` | Todas las transiciones |
  | `--table-header-bg` | `#17324d` | Fondo de `<th>` en tablas |
  | `--table-header-text` | `#ffffff` | Texto de `<th>` en tablas |
  | `--table-border` | `#d7dee8` | Bordes de celdas |
  | `--table-row-bg` | `#ffffff` | Fondo de filas de tabla |
  | `--table-row-alt` | `#f4f7fb` | Filas pares alternas |
  | `--table-row-hover` | `#fff3e6` | Hover de filas (tint naranja) |
  | `--table-text` | `#27313f` | Texto de celdas |
  | `--card-bg` | `#ffffff` | Fondos de cards |
  | `--border-color` | `#dee2e6` | Bordes generales |

  ### Valores hardcodeados candidatos a extracción (Prioridad)

  | Valor | Ocurrencias | Variable propuesta |
  |-------|-------------|---------------------|
  | `#f8f9fa` | 15+ | `--bg-subtle` |
  ...
  | `8px` (border-radius) | 30+ | `--radius-sm` (distinto de `--radius`) |
  | `0.85rem` / `0.9rem` | 20+ | `--font-size-sm` / `--font-size-base` |
  | `14px` / `16px` (padding) | 25+ | `--spacing-md` / `--spacing-lg` |

  ---

  ## Inventario de Componentes Shared

  ### Componentes actuales en `Shared/`
  ...
  | `ComunicadosView.jsx` | Formulario de visualización de comunicados | Multi-rol |
  | `ConfirmDeleteModal.jsx` | Modal de confirmación de eliminación | Multi-rol |
  | `DiagnosticosView.jsx` | Vista de diagnósticos | Multi-rol |
  | `FormModal.jsx` | Wrapper estándar de modal | Multi-rol |
  | `LoadingScreen.jsx` | Pantalla de estado de carga | Multi-rol |
  | `LoadingSpinner.jsx` | Spinner compacto | Multi-rol |
  | `Logo.jsx` | Componente logo de la app | Multi-rol |
  | `SeleccionRol.jsx` | Pantalla de selección de rol | Multi-rol |

  ### Matriz de prioridad de extracción

  | Candidato | Esfuerzo | Impacto | Prioridad |
  |-----------|----------|---------|-----------|
  | `StatCard` | Bajo | Alto (usado en 5+ paneles) | **P0** |
  | Consolidación `FiltrosAnioCurso` | Medio | Alto (2 duplicados existentes) | **P0** |
  | Patrón `TablaCRUD` | Alto | Alto (elimina 15+ duplicados) | **P1** |
  | Patrón `ProfileGrid` | Medio | Medio (5 componentes panel) | **P1** |
  | Componente `EmptyState` | Bajo | Bajo (simple pero repetido) | **P2** |

  ### Anti-patrones a prevenir

  | Anti-patrón | Descripción | Prevención |
  |-------------|-------------|------------|
  | Componentes shared duplicados | Mismo componente existe en `Shared/` y en una carpeta de rol | Forzar importación desde `Shared/` únicamente |
  | Componentes Dios | Componente shared con 15+ props para cada caso de uso | Mantener props enfocadas; preferir composición sobre configuración |
  | Componentes shared con lógica de rol | Componente que verifica `user.role` internamente | Siempre recibir el rol como prop; mantener los shared agnósticos al rol |
  | Componentes shared sin uso | Componente shared que ya no es usado por ningún rol | Auditar trimestralmente con verificación de imports no usados |
  | Fuga de CSS | Nombres de clases CSS de componentes shared que entran en conflicto con CSS de roles | Usar nomenclatura tipo BEM: `.shared-{component}__{element}` |

  ---

  ## Checklist de Testing Visual

  ### Verificación de cumplimiento de componentes de formulario

  #### A. Overlay oscuro / Contenedor

  - [ ] Los formularios **modal** usan `ddjj-modal-overlay` como wrapper exterior.
  - [ ] Los formularios **inline** usan uno de: `inline-form-container`, `administradores-inline-form`, `proyecto-form` o `acta-form`.
  - [ ] **Sin contenedores de formulario con fondo blanco.** El fondo debe ser `--sidebar-hover` (`#163452`) o su variante semitransparente.
  - [ ] La opacidad del overlay es `rgba(15, 23, 42, 0.62)` — nunca opaco, nunca transparente.

  #### B. Estructura del formulario (Modal estándar)

  - [ ] Exterior: `ddjj-modal-overlay` → `standard-modal` → `standard-modal-header` / `standard-modal-body` / `standard-modal-footer`.
  - [ ] Header: `<h3>` con texto blanco, padding `18px 24px`, borde inferior `rgba(255,255,255,0.08)`.
  - [ ] Body: padding `24px`, `overflow: auto`, `flex: 1`.
  - [ ] Footer: `display: flex; justify-content: flex-end; gap: 10px; padding: 16px 24px;` con borde superior que coincida con el header.
  - [ ] Botones del footer: "Cancelar" (`btn-secondary`) a la izquierda, "Guardar" (`btn-primary`) a la derecha.

  #### C. Labels

  - [ ] Cada `<input>` tiene un `<label htmlFor="...">` correspondiente.
  - [ ] El color del label es `#cbd5e1` (sobre fondos oscuros) o heredado (sobre fondos claros).
  - [ ] Tamaño del label: `0.85rem`, peso `500`.
  - [ ] Nunca usar placeholder como label.

  #### D. Inputs / Selects / Textareas

  - [ ] Envolueltos en `.form-group-filter`.
  - [ ] Fondo: `var(--table-row-bg)` (blanco).
  - [ ] Borde: `1px solid var(--table-border)`, radio `10px`.
  - [ ] Padding: `12px 14px`.
  - [ ] Focus ring: `border-color: var(--primary-color)` + `box-shadow: 0 0 0 3px rgba(253, 126, 20, 0.10)`.
  - [ ] Color del placeholder: `#6b7785`.
  - [ ] Textarea: `resize: vertical`, `min-height: 170px`.

  #### E. Grid / Layout del formulario

  - [ ] Layout de dos columnas usa `preceptor-form-row--two` (`grid-template-columns: repeat(2, minmax(0, 1fr))`).
  - [ ] Campo de ancho completo usa `preceptor-form-full` (`grid-column: 1 / -1`).
  - [ ] Grid asimétrico usa `preceptor-form-grid` (`1fr / minmax(160px, 220px)`).
  - [ ] Espacio entre campos: `14px`.
  - [ ] Secciones separadas por `preceptor-form-section` (borde superior + padding-top).

  #### F. Botones

  - [ ] Acción primaria: `btn btn-primary` (naranja `#fd7e14`, texto blanco).
  - [ ] Acción cancelar: `btn btn-secondary` (gris `#6c757d`, texto blanco).
  - [ ] Acción destructiva: `btn btn-danger` (rojo `#dc3545`).
  - [ ] Sin colores o estilos de botón inventados.
  - [ ] Estado de carga: `disabled={saving}`, el texto cambia a "Guardando...".

  #### G. Mensajes de error / éxito

  - [ ] Errores renderizados inline vía estado, nunca vía `alert()`.
  - [ ] Estilo de error: `.error-message` o `<p>` vía estado con `color: #b91c1c`.
  - [ ] Estilo de éxito: `<p>` vía estado con `color: #15803d` o `#1e7e34`.
  - [ ] Mensajes ubicados encima del formulario o debajo del campo correspondiente.

  #### H. Comportamiento responsive

  - [ ] En `768px`: el grid del formulario colapsa a una sola columna.
  - [ ] En `768px`: los botones pasan a ancho completo.
  - [ ] El ancho máximo del modal respeta el viewport: `min(1100px, 100%)`.
  - [ ] La altura máxima del modal: `min(92vh, 900px)`.

  ### Checklist de cumplimiento UX

  - [ ] **Sin llamadas a `alert()`** en ningún componente.
  - [ ] **`window.confirm()`** usado para todas las eliminaciones.
  - [ ] **`refreshData()`** llamado después de cada operación CRUD.
  - [ ] **Sin React Router** — navegación solo por estado.
  - [ ] **`aria-label`** en todos los botones que solo tienen icono.
  - [ ] **`aria-hidden="true"`** en todos los `<i>` decorativos.
  - [ ] **`htmlFor`** en todos los elementos `<label>`.
  - [ ] **Orden del sidebar preservado:** Mi Perfil primero, Cerrar Sesión último.

  ### Checklist de auditoría responsive

  - [ ] **Todos los grids colapsan a una sola columna en 768px.** Verificar que `grid-template-columns` se sobreescribe.
  - [ ] **Las tablas tienen scroll horizontal.** `.table-responsive` con `overflow-x: auto` y `min-width: 650px` en `<table>`.
  - [ ] **El sidebar se transforma correctamente.** En 768px: fijo en la parte inferior, layout horizontal, brand oculto.
  - [ ] **Los botones pasan a ancho completo en 768px** (excepto `.btn-sm`).
  - [ ] **Las filas de filtros se apilan verticalmente en 768px.** `.filter-row` → `flex-direction: column`.
  - [ ] **Las cards reducen padding.** `30px` → `20px` (768px) → `15px` (480px).
  - [ ] **Sin desbordamiento horizontal.** `overflow-x: hidden` en `html, body` en móvil.
  - [ ] **El padding inferior compensa el sidebar móvil.** `padding-bottom: 95px` en `.main-content` en 768px.

  ---

  ## Mapa de Secciones del CSS

  ### Mapa de secciones de `index.css` por rango de líneas

  | Rango de líneas | Sección |
  |-----------------|---------|
  | 1–25 | Variables `:root` |
  | 27–32 | Reset global (`*`) |
  | 34–40 | `body` |
  | 42–64 | Utilidades de botones (`.btn-*`) |
  | 66–84 | Utilidades de alertas |
  | 86–237 | Pantalla de login |
  | 239–398 | Layout del dashboard (sidebar, tabs, main content) |
  | 400–414 | Header principal |
  | 416–432 | Info de perfil de usuario / avatar |
  | 438–456 | Estado vacío de notificaciones |
  ...
  | 1882–1995 | Redefinición de header/cards/tables (duplicados) |
  | 1999–2035 | Redefinición responsive 992px (duplicado) |
  | 2037–2202 | Redefinición responsive 768px (duplicado) |
  | 2204–2340 | Vista de horarios (grid de horarios) |
  | 2343–2480 | Modal estándar + contenedores de formularios oscuros |
  | 2487–2643 | Calendario institucional |
  | 2645–2753 | Notificaciones toast |
  | 2755–2892 | Formulario de adelantos + grilla de módulos + opciones radio |
  | 2913–2964 | Modal de confirmación de eliminación |
  | 2966–3034 | Pantalla de carga |
  | 3036–3120 | Pantalla de selección de rol |

  ---

  ## Convenciones de Nomenclatura CSS

  ### Referencia de convenciones de nomenclatura de clases

  | Patrón | Convención | Ejemplo |
  |--------|-----------|---------|
  | Raíz del componente | `.kebab-case` | `.sidebar-brand` |
  | Modificador | `--modifier` | `.preceptor-form-row--two` |
  | Elemento BEM | `__element` | `.confirm-modal__title` |
  | Modificador BEM | `--modifier` | `.modulo-card--checked` |
  | Utilidad | `.type-value` | `.mt-16`, `.text-center`, `.flex-row` |
  | Estado | `.component--state` | `.toast--saliendo`, `.loading-screen--dark` |
  | Nombres en español | Siempre | `.asistencia-badge`, `.campo-obligatorio` |
