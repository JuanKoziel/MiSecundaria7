...
| Familia (Tutor/Padre) | Consulta de calificaciones, asistencias, comunicados, horarios, boletín, actas de sus hijos | Usuario de solo lectura/consulta; necesita información clara y oportuna |
| Alumno (Estudiante) | Consulta de calificaciones, asistencias, horario, comunicados, boletín PDF, materias adeudadas | Usuario final estudiante; UX simple, mobile-first, autogestión básica |
...
| 3 | ¿Qué información registran actualmente por rol? | Campos que capturan: alumno (DNI, curso, tutor), docente (DDJJ, materia), preceptor (asistencia, notas), admin (usuarios, cursos) | Definir modelo de datos mínimo y migración |
...
| 9 | ¿Qué tipos de usuarios/roles existirán? | preceptor, jefe_preceptores, admin, director, docente, familia, alumno | Diseñar RBAC y navegación por rol |
...
| 12 | ¿Qué información puede crear/modificar/eliminar cada rol? | Permisos de escritura por entidad: alumno, nota, asistencia, acta, comunicado | Definir reglas de autorización y auditoría |
...
| 16 | ¿Cómo se gestiona la inscripción de alumnos a cursos? | Proceso: manual, automática, por tutores, lista de espera | Flujo de alta/baja de alumnos |
...
| 21 | ¿Cómo se comunican novedades a familias y alumnos? | Canales: sistema, email, WhatsApp, papel; alcances: general, curso, materia, alumno | Módulo de comunicados con alcances |
...
| # | Pregunta | Información que se Busca | Importancia |
|---|----------|--------------------------|-------------|
| 25 | ¿Qué ocurre si un alumno cambia de curso/división durante el año? | Reasignación de notas, asistencias, comunicados, tutores | Integridad referencial y migración de datos |
...
| 29 | ¿Cómo se gestionan alumnos con situaciones especiales? | Inclusión, adaptaciones curriculares, certificados médicos | Datos sensibles y visibilidad restringida |
...
| # | Pregunta | Información que se Busca | Importancia |
|---|----------|--------------------------|-------------|
| 31 | ¿Qué datos personales se almacenan por tipo de usuario? | Alumno: DNI, fecha nac, dirección, teléfono, tutor, médica. Docente: DDJJ, título, cargo. Familia: vínculo, contacto | Cumplimiento Ley 25.326 (Datos Personales) |
| 32 | ¿Quién puede acceder a cada tipo de dato? | Matriz de acceso: admin (todo), preceptor (sus cursos), docente (sus materias), familia (sus hijos), alumno (suyo) | Principio de menor privilegio |
...
| 41 | ¿Qué dispositivos utilizarán principalmente cada rol? | Preceptor/Admin: desktop. Docente: desktop/notebook. Familia/Alumno: móvil (celular) | Responsive / PWA / mobile-first por rol |
| 42 | ¿Desde qué contextos se usa el sistema? | Oficina (preceptor/admin), aula (docente), hogar (familia/alumno), movimiento (preceptor en patio) | Diseño contextual y offline-first |
...
| 46 | ¿Hay normativas provinciales/nacionales que obliguen formatos? | Boletín oficial, actas modelo, planificaciones obligatorias, reportes estadísticos | Cumplimiento normativo |
...
| Roles implementados | preceptor, jefe_preceptores, admin, director, docente, familia, alumno | `AuthContext.jsx`, `App.jsx` |
...
| Entidades principales | Alumnos, Docentes, Preceptores, Directivos, Cursos, Materias, Curso-Materia, Calificaciones, Asistencias, Actas, Comunicados, Notificaciones, Horarios, Planificaciones, Libro Temas, Suplencias, Adelantos Horas, Diagnósticos, Padres/Tutores, Inscripciones, Períodos, Ciclos Lectivos, Eventos Institucionales | `DataContext.jsx`, `api.js` |
| Funcionalidades core implementadas | Login multi-rol, selector de rol, dashboard por rol, sidebar acordeón, header estándar, notificaciones push (polling 30s), navegación desde notificaciones, filtros año/curso, carga batch de notas, gestión de suplencias, boletín PDF, historial académico | Código fuente completo |
...
- Alcances reales de comunicados (curso, materia, alumno, general) y adjuntos (P21)
...
- Formatos obligatorios de boletín, actas, planificaciones (P46)
...
| "Los comunicados tienen alcances flexibles (curso, materia, alumno)" | `comunicados` con `alcances[]` en `DataContext` | Puede ser solo general o solo curso | P21 |
...
| "El boletín PDF usa formato oficial de la provincia" | `boletinHTML` + `exportarBoletinPDF` en `utils/boletin.js` | Formato puede requerir ajustes legales | P46 |
...
| "El sistema es mobile-first para familia/alumno" | `AlumnoDashboard` y `FamiliaDashboard` usan header/sidebar responsive | Puede necesitar PWA o app nativa | P41 |
...
| **Carga de calificaciones sin validación de reglas de negocio en frontend** | `guardarCalificacionesBatch` acepta cualquier payload; `PanelActividades`/`Notas` no validan rango, decimal, diagnóstico obligatorio | Errores en boletines, reclamos, reprocesos |
...
| **Historial académico (boletín) sin versión oficial firmada** | `getBoletinAcademico` + PDF generado en cliente | PDF manipulable, sin validez legal |
...
| **Acuse de recibo de comunicados** (confirmación lectura por familia/alumno) | Funcional | Media | Familia, Alumno, Preceptor, Admin | `ComunicadosView`, `Comunicados` |
| **Notificación automática en suplencias** (a docente titular, preceptor, alumnos) | Funcional | Media | Docente, Preceptor, Alumno | `Suplencias`, `Notificaciones` |
| **Boletín oficial con firma digital / código de verificación** | Funcional / Legal | Alta | Admin, Familia, Alumno | `boletinHTML`, `exportarBoletinPDF`, `getBoletinAcademico` |
...
| **PWA / App móvil nativa** para familia y alumno | No Funcional | Media | Familia, Alumno | `FamiliaDashboard`, `AlumnoDashboard` |
| **Integración SSO (Google / Microsoft / SAML)** | No Funcional | Baja | Todos | `AuthContext`, `login` |
| **Import/Export masivo** (alumnos, docentes, notas, horarios) desde Excel/CSV | Funcional | Media | Admin, Preceptor | `Alumnos`, `Docentes`, `Notas`, `Horarios` |
...
| **Asistencia** | ¿Existe "asistencia docente" separada de la de alumnos? | `asistencias-docentes/` endpoints sugieren que sí; ¿cómo se usa? |
...
| **Actas** | ¿Las actas de alumno (`acta-alumno`) y docente (`acta-docente`) firman el mismo documento o son actas distintas? | Modelo de datos y flujo de firma |
...
| **Materias Adeudadas** | ¿La inscripción a rendición (`rendirMateriaAdeudada`) la hace el alumno o el preceptor/admin? | `PanelMateriasAdeudadasAlumno` sugiere autogestión |
...
4. **Trazabilidad legal es prioridad**: Actas, notas, boletines, DDJJ requieren auditoría inmutable (`/historial/`). Sin confirmar requisitos legales (P35, P37, P46), el sistema carece de validez oficial.

5. **Familia y Alumno son "mobile-first" por contexto de uso** (hogar, celular). El código usa header/sidebar responsive, pero no hay PWA, service worker ni offline. Validar si es suficiente (P41, P42).
...
7. **Migración de datos es el mayor riesgo técnico**: Alumnos, docentes, cursos, notas históricas, actas previas. Sin plan de migración validado (P1, P3, P4), el sistema nace sin historia.

8. **Normativa provincial define formatos obligatorios** (boletín, actas, planificaciones). El código genera PDF propio (`boletinHTML`). El relevamiento debe traer los modelos oficiales (P46).
